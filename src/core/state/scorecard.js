...
const ScorecardDataState = selector({
  ...
  get: async ({ get }) => {
    ...
    const periods = get(ScorecardViewSelector("periodSelection"))?.periods.map(
      ({ id }) => id
    );
    ...
    const indicators = flattenDeep(
      flattenDeep(dataGroups.map(({ dataHolders }) => dataHolders))?.map(
        ({ dataSources }) => dataSources
      )
    );

    // const data = get(ScorecardAnalytics("scorecardAnalytics"));
...
const ScorecardConfigStateSelector = selectorFamily({
  ...
  set:
    ...
    ({ set, get }, newValue) => {
      const scorecardId = get(ScorecardIdState);
      set(ScorecardConfState(scorecardId), (prevState) =>
        _set(cloneDeep(prevState), path, newValue)
      );
    },
});
...
const ScorecardViewState = atom({
  ...
  default: selector({
    ...
    get: ({ get }) => {
      const scorecardId = get(ScorecardIdState);
      const { orgUnitSelection, options, periodType } =
        get(ScorecardConfState(scorecardId)) ?? {};
...
const ScorecardViewSelector = selectorFamily({
  ...
  get:
    (path) =>
    ({ get }) =>
      _get(get(ScorecardViewState), path),
  set:
    (path) =>
    ({ set }, newValue) =>
      set(ScorecardViewState, (prevState) =>
        _set(cloneDeep(prevState), path, newValue)
      ),
});
