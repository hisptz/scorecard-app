    ...
    return (
        <div className="container p-16 data-source-form-container">
            <div className="column">
                <RHFCustomInput disabled mandatory valueType={DHIS2ValueTypes.TEXT.name}
                                label={i18n.t("Name")} name={`${path}.displayName`}/>
                <RHFCustomInput mandatory valueType={DHIS2ValueTypes.TEXT.name} label={i18n.t("Label")}
                                name={`${path}.label`}/>
                <RHFCustomInput valueType={DHIS2ValueTypes.NUMBER.name} label={i18n.t("Weight")}
                                name={`${path}.weight`}/>
                <div className="row space-between">
                    <div className="column pr-16 effective-gap-settings">
                        <RHFCustomInput valueType={DHIS2ValueTypes.NUMBER.name}
                                        label={i18n.t("Effective Gap")} name={`${path}.effectiveGap`}/>
                    </div>
                    <div className="column pl-16 indicator-options-settings-area">
                        <RHFCustomInput valueType={DHIS2ValueTypes.TRUE_ONLY.name}
                                        label={i18n.t("Display Arrows")}
                                        name={`${path}.displayArrows`}/>
                        <RHFCustomInput valueType={DHIS2ValueTypes.TRUE_ONLY.name}
                                        label={i18n.t("High is Good")} name={`${path}.highIsGood`}/>
                        <RHFCustomInput valueType={DHIS2ValueTypes.TRUE_ONLY.name}
                                        label={i18n.t("Show Colors")} name={`${path}.showColors`}/>
                    </div>
                </div>
                <Divider/>
                <div className="row">
