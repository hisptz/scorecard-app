...
export default function LevelTargetsField({
    ...
    const value = useMemo(() => !Array.isArray(initialValue) ? initialValue : fromPairs([...orgUnitLevels?.map(({id}) => [id, generateLegendDefaults(multipleFields, weight, highIsGood)])]), [highIsGood, initialValue, multipleFields, orgUnitLevels, weight]);
                            ...
                            <div className='column w-100'>
                                {
                                    multipleFields?.map((field, index) => {
                                        ...
                                        return <CustomInput key={`level-${id}-${index}`} valueType={field.valueType}
                                                            input={input} {...field}/>
                                    })
                                }
                            </div>
