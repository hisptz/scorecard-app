...
export default class DataSets extends DataSource {
    ...
    async getDataSources(engine, {filter, page}) {
        const response = await engine.query(this.groupQuery, {variables: {filter, page}})
        return {
            pager: response?.dataSets?.pager,
            data: flattenDeep(response?.dataSets?.dataSets?.map(({
                                                                     id,
                                                                     displayName
                                                                 }) => (this.categories.map(({
                                                                                                 id: categoryId,
    ...
    async getGroups(engine) {
        const response = await engine.query(this.groupQuery, {variables: {filter: null, page: 1}})
