...
export default function GroupSelector({selectedDataType, onSelect, selectedGroup}) {
    ...
    return (
        <div className='pb-8'>
            <SingleSelectField clearable selected={selectedGroup?.id} onChange={({selected: newValue}) => {
                const selectedGroup = find(groups, ['id', newValue])
                onSelect(selectedGroup)
            }} error={error} validationText={error?.message} loading={loading} disabled={disabled}
                               label={selectedDataType.groupResource === 'programs' ? 'Select Program' : 'Select Group'}>
                {
                    groups?.map(({displayName, id}) => {
                        return (
                            <SingleSelectOption key={id} value={id} label={displayName}/>
                        )
                    })
                }
            </SingleSelectField>
