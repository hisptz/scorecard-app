...
import {generateScorecardSummary} from "../utils/scorecard";
        ...
        if (!isEmpty(scorecards)) {
            ...
            if (!isEmpty(scorecardsToRestore)) {
                ...
                for (const scorecardId of scorecardsToRestore) {
                    const {scorecard} = await engine.query(singleScorecardQuery, {variables: {id: scorecardId}})
                    const scorecardSummary = generateScorecardSummary(scorecard)
                    updatedSummary.push(scorecardSummary)
                }
                await engine.mutate(restoreMutation, {variables: {data: updatedSummary}})
            }
        }
    }
}
