...
import {
	dataGroupSchema,
	dataHolderSchema,
	organisationUnitSelectionSchema,
	OrgUnitSelection,
	scorecardConfig,
	scorecardSharing,
	scorecardViewOptionsSchema,
...
export function useFormSchema() {
	...
	return scorecardConfig.extend({
		title: z
			.string({ required_error: i18n.t("Title is required") })
			.min(4, i18n.t("Title must have at least 4 characters"))
			.refine(
				async (value) => {
					return await checkTitleAvailability({
						engine,
						id,
						title: value,
					});
				...
				{
					message: i18n.t(
						"A scorecard with this title already exists. Please select another title"
					),
				}
			),
		dataSelection: z.object({
			dataGroups: z
				.array(
					dataGroupSchema.extend({
						dataHolders: z
							.array(dataHolderSchema)
							.min(
								1,
								i18n.t(
									"A data group must have at least one data item"
								)
							),
					})
				)
		...
		orgUnitSelection: organisationUnitSelectionSchema.refine(
			(value) => {
				return anyOrgUnitSelected(value);
			},
			{
				message: i18n.t(
					"You must select at least one organisation unit"
				),
			}
		),
