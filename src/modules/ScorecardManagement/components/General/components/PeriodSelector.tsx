...
export function PeriodSelector() {
	...
	const selectedPeriods = ((field.value as Array<{ id: string }>) ?? []).map(
		({ id: periodId }) => {
	...
	const buttonLabel = !isEmpty(selectedPeriods)
		? i18n.t("Change")
		: i18n.t("Select");
	...
	const filteredPeriodTypes = useMemo(() => {
		...
		return uniqBy(
			...
			.filter((periodType) => periodType.id != periodTypeId)
			.map((periodType) => periodType.id);
	}, [periodTypeId]);
		...
		<>
			...
			<Field
				...
				helpText={
					!isEmpty(selectedPeriods)
						? `${i18n.t("Selected")}: ${selectedPeriodNames.join(
								", "
						  )}`
						: ""
				}
			>
				<Button onClick={onShow}>
					{i18n.t("{{buttonLabel}} default period", { buttonLabel })}
				</Button>
			</Field>
		</>
	);
