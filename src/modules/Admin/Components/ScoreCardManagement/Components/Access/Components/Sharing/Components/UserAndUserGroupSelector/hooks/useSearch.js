...
export default function useSearchUserAndUserGroup(initialKeyword) {
    ...
    const {data, errors, loading, refetch} = useDataQuery(query, {variables: {keyword}, lazy: true})
    ...
    const userAndUserGroups = useMemo(() => {
        const users = data?.users?.users?.map(user=>({...user, type: 'user'})) || []
        const userGroups = data?.userGroups?.userGroups?.map(userGroup=>({...userGroup, type: 'userGroup'})) || []
