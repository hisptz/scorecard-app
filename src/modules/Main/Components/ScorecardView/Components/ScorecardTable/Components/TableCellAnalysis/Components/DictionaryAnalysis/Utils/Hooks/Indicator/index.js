...
export function useGetFormulaDataDetailed(formula, engine, location) {
  ...
  useEffect(() => {
    ...
    let tempArr2 = getFormulaSources(
      formula,
      dataTypesInitials.PROGRAM_INDICATOR
    );
    let tempArr3 = getFormulaSources(
      formula,
    ...
    let tempArr7 = getFormulaSources(
      formula,
      dataTypesInitials.PROGRAM_DATA_ELEMENT
    );

    async function fetch() {
      tempArr = await getWordData(
        engine,
        tempArr,
        dataTypes.DATA_ELEMENT,
        location
      );
      tempArr2 = await getWordData(
        engine,
        tempArr2,
        dataTypes.PROGRAM_INDICATOR,
        location
      );
      tempArr3 = await getWordData(
        engine,
        tempArr3,
        dataTypes.DATASET_REPORTING_RATES,
        location
      );
      tempArr4 = await getWordData(
        engine,
        tempArr4,
        dataTypes.ATTRIBUTES,
        location
      );
      tempArr5 = await getWordData(
        engine,
        tempArr5,
        dataTypes.CONSTANTS,
        location
      );
      tempArr6 = await getWordData(
        engine,
        tempArr6,
        dataTypes.ORG_UNIT_COUNT,
        location
      );
      tempArr7 = await getWordData(
        engine,
        tempArr7,
...
async function getWordData(engine, arr, type, location) {
