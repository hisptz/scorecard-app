            ...
            if (!isEmpty(source) && !isEmpty(destination)) {
                if (source?.droppableId === destination?.droppableId) {
                    onLayoutChange(prevLayout => {
                        const newList = updateListFromDragAndDrop(prevLayout[source.droppableId], source?.index, destination?.index)
                        return {
                            ...prevLayout,
                    ...
                    onLayoutChange(prevLayout => {
                        ...
                        } = updateListsFromDragAndDrop({sourceList, destinationList}, {
                        ...
                        <DroppableArea onDrop={onDrop} type={column ? 'column' : 'series'}>
                            {
                                (column ?? series)?.map((val, index) => (
                                    <DraggableItem index={index} key={`${val}`} type={val}>
                                        <Chip icon={LAYOUTS?.[val]?.icon}
                                              className={'w-100'}>{LAYOUTS?.[val]?.displayName}</Chip>
                                    </DraggableItem>
                                ))
                            }
                        </DroppableArea>
