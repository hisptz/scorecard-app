...
export default function LegendsAnalysis({ id }) {
  ...
  if (data?.legendAnalysis?.legendSets?.length === 0) {
    return (
      <>
        <p>
          {i18n.t(
            "There are no legends associated with these Program Indicator"
          )}{" "}
        </p>
      </>
    ); //no legends sets
  ...
  return (
    <div>
      ...
      <p>
        {i18n.t(
          "Uses {{variables}} legends for for analysis, spread across multiple cut-off points of interest, existing legends are:",
