...
export default function SelectionWrapper({selectedItems, onClick, icon, label}) {
    const Icon = icon
        ...
        <Tooltip content={<div>
            {
                selectedItems?.map(({displayName, name, id}) => (
                    <p style={{margin: 4}} key={`cell-analysis-selector-${id}`}>{name ?? displayName}</p>
                ))
            }
        </div>}>
            <div className='row align-items-center' style={{
                ...
                {
                    selectedItems !== null &&
                    <p style={{fontSize: 14, color: colors.grey700, margin: 0, paddingLeft: 4}}>
