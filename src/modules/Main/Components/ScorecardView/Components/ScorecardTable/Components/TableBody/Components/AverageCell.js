...
export default function AverageCell({value, bold}) {
    ...
    if (typeof (value) === 'number') {
        return <td className='data-cell' align="center">
    ...
    return (
        <td className='data-cell' align="center">
            {
                values.length > 1 ?
                    <LinkedCellSvg bold topValue={head(values) ? round(+head(values), 2) : ''}
                                   bottomValue={last(values) ? round(last(values), 2) : ''}/> :
                    <SingleCellSvg bold={bold} value={head(values) ? round(head(values), 2) : ''}/>
            }
        </td>
    )
}
