    ...
    try {
        const json = JSON.stringify({
            organisationUnits: orgUnits?.map(({id, level, displayName, path}) => {
                ...
                const parent = pathArray.length >= 2 ? pathArray?.[pathArray.length - 2] ?? '' : ''
                return {
                    id,
                    name: displayName,
            ...
            indicators: flatten(dataHolders?.map(({dataSources}) => dataSources?.map(({id, displayName}) => ({
                id,
                name: displayName
            }))))
        })
