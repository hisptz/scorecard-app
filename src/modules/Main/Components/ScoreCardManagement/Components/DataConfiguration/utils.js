...
export function updateLegendsOnDataGroups(dataGroups, {targetOnLevels, filteredLegendDefinitions, orgUnitLevels}) {
    const newValue = targetOnLevels
        ? fromPairs([...orgUnitLevels?.map(({id}) => [id, generateLegendDefaults(filteredLegendDefinitions, 100, true)])])
    ...
    for (const groupIndex in updatedGroups) {
        for (const holderIndex in updatedGroups[groupIndex]?.dataHolders) {
            for (const sourceIndex in updatedGroups[groupIndex]?.dataHolders?.[holderIndex]?.dataSources) {
