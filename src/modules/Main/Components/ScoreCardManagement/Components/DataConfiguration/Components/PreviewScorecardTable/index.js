            ...
            <DataTable bordered width={"100%"}>
                <DataTableHead>
                    <DataTableRow>
                        <DataTableCell fixed align='center' className='table-header scorecard-table-header' bordered
                                       rowSpan={"2"}>{i18n.t('Organisation Unit')}</DataTableCell>
                        {
                            columns?.map(column => (
                                <DataTableCell fixed align='center'
                                               className='table-header scorecard-table-header' bordered
                                        ...
                                        <tr>
                                            {
                                                group?.dataHolders?.map((({dataSources}) => (
                                                    <DataTableCell width={"100px"} fixed bordered
                ...
                <DataTableBody>
                    <DataTableRow bordered>
                        <DataTableCell width={"100%"} className={"min-width-200"} fixed tag="th" bordered>
                            {
