...
export default function SelectedDataSourceConfigurationForm() {
  ...
  const [selectedDataHolder, updateSelectedDataHolder] = useRecoilState(
    ScorecardConfigDirtySelector({
      key: "dataSelection",
      path,
    })
    ...
    (index) =>
      ({ values, dirty }) => {
        if (dirty) {
          ...
          if (
            prevValue.weight !== values.weight ||
            prevValue.highIsGood !== values.highIsGood
          ) {
            values.legends = targetOnLevels
              ? fromPairs([
          ...
          updateSelectedDataHolder((prevState) =>
            ScorecardIndicator.set(prevState, "dataSources", updatedList)
          );
        }
      },
    [
      filteredLegendDefinitions,
      orgUnitLevels,
      selectedDataHolder?.dataSources,
      targetOnLevels,
    ]
  );
  return (
