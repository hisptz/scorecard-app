...
export default function useSetDefaults() {
    ...
    const resetLegends = useRecoilCallback(({snapshot, set}) => () => {
        const {dataGroups, orgUnitLevels, legendDefinitions} = snapshot.getLoadable(waitForAll({
        ...
        const filteredLegendDefinitions = filter(legendDefinitions, ({isDefault}) => (!isDefault))
        if (!isEmpty(dataGroups)) {
            ...
            set(ScorecardConfigDirtySelector({
                key: "dataSelection",
                path: 'dataGroups'
            }), updatedGroups)
        }
