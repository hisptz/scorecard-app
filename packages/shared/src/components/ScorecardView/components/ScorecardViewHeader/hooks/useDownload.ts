...
export default function useDownload(downloadRef: any, dataEngine: any) {
	...
	const orgUnitsIds = useMemo(() => orgUnits?.map(({ id }: any) => id), [
		orgUnits,
	]);
	...
	const downloadFunction = (type: any) => {
		if (
			loading !== undefined &&
			!loading &&
			orgUnitState.state === "hasValue"
	...
	const download = (type: any) => {
		const error: any = downloadFunction(type);
		if (error) {
			show({
