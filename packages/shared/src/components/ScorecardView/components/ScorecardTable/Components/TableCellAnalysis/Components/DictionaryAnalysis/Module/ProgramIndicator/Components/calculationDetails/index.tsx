...
export default function CalculationDetails({ id }: any) {
	...
	const res: any = data?.calculation;
	...
	return (
		<div>
			...
			<p>
				{i18n.t(
					"Calculation of the values will be {{variables}} of {{variables2}} across orgunit and period.",
					{
			...
			<div>
				<p>
					{i18n.t(
						"Below are period boundaries that determines which {{variables}} will be included in calculations of the program indicators, where for",
						{
							variables: lowerCaseAllWordsExceptFirstLetters(
								res?.analyticsType,
							),
						},
					)}
