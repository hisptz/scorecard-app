...
export default function GroupsHeaderRow({ nested, orgUnits }: any) {
	const { dataGroups } =
	...
	return (
		!isSingleGroup && (
			<DataTableRow className={classes["table-header-row"]}>
				...
				{dataGroups?.map(({ title, id, dataHolders }: any, index: number) => (
					<DataTableCell
						fixed
						className="scorecard-table-header header"
						align="center"
						bordered
						width={`${(dataHolders?.length ?? 1) *
							(periods?.length ?? 1) *
							ScorecardTableConstants.CELL_WIDTH}px`}
						top={"0"}
