...
export default function HoldersHeaderRow({ orgUnits, nested }: any) {
	const { dataGroups } =
	...
	return (
		<DataTableRow className={classes["table-header-row"]}>
			{isSingleGroup && (
				<OrgUnitHeaderCells nested={nested} orgUnits={orgUnits} />
			)}
			{dataGroups?.map(({ dataHolders }: any) =>
				dataHolders?.map(({ id, dataSources }: any, index: number) => (
					<DataTableColumnHeader
