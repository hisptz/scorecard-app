...
function checkForEventDataType(favoriteObject: any, favoriteType: string) {
...
function getTableTitle(favoriteObject: any, interventionName: any) {
	return `${favoriteObject.title ||
		favoriteObject.displayName ||
		favoriteObject.name}${interventionName ? ` : ${interventionName}` : ""}`;
}
