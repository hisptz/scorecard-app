...
export default function Programs({ sources }: any) {
	...
	const updateProgramDataElements = useSetRecoilState(
		indicatorGroupProgramDataElements,
	);

	const { loading, error, data } = useGetIndicatorProgramSource(
		sources,
		engine,
	);
	let res = [];
	let allProgram: any = [];
	useEffect(() => {
		updateProgramDataElements((prev) => {
	...
	if (res?.length > 0) {
		return (
			<div>
				Programs
				<ul>
					{allProgram?.map((dt: any, index: any) => {
						return (
							<li key={index}>
								<b>{dt?.displayName}</b>{" "}
								{i18n.t(
