...
function childrenAlreadyDisplayed(orgUnit: any, childrenOrgUnit: any) {
	return childrenOrgUnit.some(
		(childOrgUnit: any) => childOrgUnit.level === orgUnit.level + 1,
	);
}
...
export default function ScorecardTableBody({
	...
	const { loading: configLoader } = useTableConfig(
		dataEngine,
		orgUnits,
		nested,
	);
	...
	const [overallAverage, setOverallAverage] = useState<any>();
						...
						<Fragment>
							{filteredOrgUnits?.map((orgUnit, index) => (
								<>
									...
									{orgUnit.level === lowestOrgUnitLevel.level ||
									childrenAlreadyDisplayed(orgUnit, childrenOrgUnits) ? (
										<ParentOrgUnitRow
											index={index}
							...
							{childrenOrgUnits?.map((orgUnit: any, index: number) => {
