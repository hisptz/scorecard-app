...
export default function TableCellAnalysis({
	...
	const setStates = useRecoilCallback(
		({ set, snapshot }) => () => {
			const orgUnitOptionValue = snapshot.getLoadable(
				orgUnitSelectorOptionOnCell,
			).contents;
			const lowestOrgUnitLevel = snapshot.getLoadable(LowestOrgUnitLevel)
				.contents;
			...
			function orgUnitUpdateSelector() {
				...
				if (orgUnitOptionValue) {
					...
					const currentOrgUnitLevel = get(orgUnitSelection, [
						"orgUnits",
						0,
						"level",
					]);
					...
					if (currentOrgUnitLevel < lowestOrgUnitLevel.level) {
						const orgUnitCurrentUserBelowLevels = find(orgUnitLevels, function(
							level,
						) {
