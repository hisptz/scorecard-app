...
const ResolvedPeriodState = selector({
	...
	get: ({ get }) => {
		...
		if (!isEmpty(periods)) {
			const relativePeriods = filter(
				get(cellPeriodOptionAtom).length > 0
					? get(cellPeriodOptionAtom)
					: periods,
				({ id }) => {
			...
			get(cellPeriodOptionAtom).length > 0
				? (allPeriods = [...differenceBy(relativePeriods, "id")])
				: (allPeriods = [...differenceBy(periods, relativePeriods, "id")]);
...
export {
	PeriodState,
	ResolvedPeriodState,
	cellPeriodOptionSelector,
	cellPeriodOptionAtom,
};
