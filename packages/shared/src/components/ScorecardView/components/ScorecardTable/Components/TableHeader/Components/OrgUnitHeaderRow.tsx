...
import {
	DraggableItems,
	ScorecardTableConstants,
} from "../../../../../../../constants";

export default function OrgUnitHeaderRow({ orgUnits, nested }: any) {
	...
	const onSortIconClick = ({ direction }: any) => {
		setSort((prevValue) => ({ ...prevValue, data: direction }));
	};

	const onDataSortClick = (direction: any) => {
		setDataSort({
			...direction,
			type: "orgUnit",
		});
	...
	return (
		<DataTableRow className={classes["table-header-row"]}>
			<DataTableCell rowSpan={"2"} fixed left={"0"} width={"50px"} />
			{itemNumber && (
