...
export default function useTableDataSources(dataEngine: any, orgUnits: any) {
				...
				return filter(prevDataSources, (value) => {
					const searchIndex = flatten(
						value.dataSources?.map(
							({ id, displayName }: any) => `${id}-${displayName}`,
						),
					).join("_");
					return searchIndex
						.toLowerCase()
				...
				if (orientation === Orientation.DATA_VS_ORG_UNIT) {
					if (dataSort.type === "orgUnit") {
						dataEngine
							...
							.subscribe((dSort: any) => (dataSourceSort = dSort));
					}
