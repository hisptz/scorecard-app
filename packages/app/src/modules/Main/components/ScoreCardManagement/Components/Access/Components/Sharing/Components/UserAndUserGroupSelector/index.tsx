...
function UserAndUserGroupsMenu({ data, onSelect, reference, onClose }: any) {
	const onSelectElement = (selected: any) => {
		onSelect(selected);
		onClose();
	};
	return reference ? (
...
export default function UserAndUserGroupSelector({
	...
	const { data, loading, error, setKeyword } =
	...
	const onKeywordChange = ({ value: keyword }: any, event: any) => {
		setValue(keyword);
		setInputReference(event?.target);
		onSearch();
	};

	const onValueSelect = (selectedValue: any) => {
