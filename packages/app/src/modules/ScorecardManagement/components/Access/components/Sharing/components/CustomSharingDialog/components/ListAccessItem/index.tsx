...
export function ListItem({
	...
	entity: { name?: string; access: string; id: string; displayName?: string };
	target: typeof AccessTypes[keyof typeof AccessTypes];
	onChange: (value: { id: string; access: string }) => void;
	onRemove?: (id: string) => void;
}) {
	const accessOptions = ACCESS_TYPES_LABELLED;
	const { name, access, id } = entity;

	const accessLabel = find(ACCESS_TYPES_LABELLED, ["value", access])?.label;
	return (
		<>
