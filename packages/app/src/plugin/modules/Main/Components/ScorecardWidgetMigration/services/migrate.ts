import { DATASTORE_SCORECARD_SUMMARY_KEY } from "@scorecard/shared";
...
export async function getOldScorecardWidgets(engine: any, keys: any) {
	if (isEmpty(keys)) {
		return [];
	}
...
export async function uploadNewScorecardWidget({
	newScorecardWidget,
	engine,
}: any) {
...
export async function getNewScorecardKeys(engine: any) {
	try {
		const { keys } =
			(await engine.query({
				keys: {
