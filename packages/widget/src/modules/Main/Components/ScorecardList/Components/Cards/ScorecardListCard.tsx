...
export default function ScorecardListCard({ scorecard, grid }: any) {
	...
	const onSelect = () => {
		confirm({
			...
			message: i18n.t(
				"Are you sure you want to select this scorecard for this dashboard?",
			),
			onConfirm: () => {
				onCreateWidget();
			},
	...
	const onCreateWidget = async () => {
		...
		return await createWidget(
			scoreCardWidget,
			useCurrentDashboardIdState,
			engineState,
		).then((response) => {
			if (
				response["widget"]["httpStatusCode"] >= 200 &&
				response["widget"]["httpStatusCode"] <= 205
			) {
				return onView();
			} else {
