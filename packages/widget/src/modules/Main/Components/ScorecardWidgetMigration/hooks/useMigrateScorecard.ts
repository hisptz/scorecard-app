...
export default function useMigrateScorecardWidget(onComplete: any) {
	...
	const onMigrationInitiated = useCallback(async () => {
		try {
			...
			const filteredNewKeys = filter(
				flattenDeep(filteredKeys),
				(oldDashboardItemId) => {
					return (
						!newDashboardKeys.includes(oldDashboardItemId) &&
						typeof oldDashboardItemId === "string"
					);
				},
			);
			if (filteredNewKeys && !isEmpty(filteredNewKeys)) {
				const oldScorecardWidgets = compact(
					await getOldScorecardWidgets(engine, filteredNewKeys),
				);
